...
var queries = { //list of mysql queries
	...
	getBookReviewsById: "SELECT book_reviews.review, book_reviews.rating, users.username, groups.name AS 'group', UNIX_TIMESTAMP(book_reviews.created_at) AS 'created_at' FROM book_reviews JOIN users ON (book_reviews.creator_id = users.id) JOIN groups ON (users.group_id = groups.id) WHERE book_id = ?",
