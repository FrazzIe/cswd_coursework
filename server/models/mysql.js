...
var queries = { //list of mysql queries
	...
	getRecentBooks: "SELECT books.id, books.isbn, books.title, books.description, books.year, books.category, GROUP_CONCAT(book_authors.author SEPARATOR ', ') AS 'authors' FROM books JOIN book_authors ON (book_authors.book_id = books.id) GROUP BY books.id ORDER BY books.id DESC LIMIT 10",
	getRandomBooks: "SELECT books.isbn, books.title, books.description, books.year, books.category, GROUP_CONCAT(book_authors.author SEPARATOR ', ') AS 'authors' FROM books JOIN book_authors ON (book_authors.book_id = books.id) GROUP BY books.id ORDER BY RAND() LIMIT 10",
